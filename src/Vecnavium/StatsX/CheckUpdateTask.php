...
class CheckUpdateTask extends AsyncTask{
    ...
    public function __construct(StatsX $plugin, bool $retry) {
        $this->retry = $retry;
    ...
    public function onCompletion(Server $server): void {
        ...
        if ($latestVersion != "" || $updateDateUnix != null || $updateUrl !== "") {
            ...
            if ($this->version !== $latestVersion) {
                $plugin->getLogger()->notice("StatsX v$latestVersion has been released on $updateDate. Download the new update at $updateUrl");
